...
void write_record(uint32_t score, int size){
	FILE *fp;
	record cur;
	record temp;	
...
void print_record(){
	...
	mvprintw(LINES/2 -5 , COLS/2 -23 , "%-8s%-18s%-10s%-10s", "Rank", "Name", "Size", "Score");
	mvprintw(LINES/2 -4 , COLS/2 -23 , "-----------------------------------------------------");
	for(i = 0 ; i < cnt ; i++)
		mvprintw(LINES/2-3+i, COLS/2 -23, "%-8d%-18s%-10d%-10d", i+1, highScore[i].name, highScore[i].size, highScore[i].score);
